...
// 设置随机验证码
const setRandomToken = (token, expires_time, email) => {
    UserModel
        .findOne({ email })
        .then((user) => {
...
// 发送验证邮件
const sendVerifyEmail = (email, expires_time, title, content) => {
    createRandomToken
        .then((token) => setRandomToken(token, expires_time, email))
        .then((token) => {
...
router.post('/signup',
    ...
    async (req, res) => {
        ...
        sendVerifyEmail(email, EXPIRES_TIME.ONEDAY, '绑定邮箱', '绑定邮箱的验证码');
    }
);
// 登录
    ...
    UserModel
        ...
        .then((user) => {
            if (!user) {
                ...
                res.status(401).json({ message: 'The email is activated.' });
...
router.post('/verifyemailtoken', async (req, res, next) => {
    ...
    if (!validator.isLength(req.body.emailToken, { min: 8 })) {
        return res.status(401).json({ message: 'The email token must be at least 8 characters long.' });
    }
    const email = req.body.email;
    UserModel
        .findOne({ email })
        ...
        .exec((err, user) => {
            ...
            if (!user || (user.emailToken != req.body.emailToken)) {
                return res.status(401).json({ message: 'Verify email token is invalid or has expired.' });
            } else {
    ...
    UserModel
        ...
        .then((user) => {
            if (!user) {
                ...
                res.json({ message: 'Please verify your email.' });
                sendVerifyEmail(email, EXPIRES_TIME.FIVEMINUTES, '重置密码', '重置密码的验证码');
            } else {
                res.status(401).json({ message: 'Please bind your email.' });
            }
        });
});
// 重置密码
